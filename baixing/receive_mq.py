...
class BaiXing(object):
    ...
    def get_phone_number(self, content):
        """过滤电话号码， 含有qq/QQ的不要， 非数字不要"""
        if content.find("q") >= 0 or content.find("Q") >= 0:
            return ""
    ...
    def getDetailContentByUrl(self, detail_content_url):
        ...
        try:
            ...
            try:
                ...
                data['company_name'] = soup.find('div', class_ = 'content').get_text().encode('utf-8')
            except Exception, e:
                data['company_name'] = ''
                pass
            ...
            data['phone_number'] = self.get_phone_number(data['phone_number'])
    ...
    def writeDB(self, data):
        ...
        cursor = self._conn.cursor()
        try:
            insert_sql = 'insert into danbaobaoxian(title, post_time, service_feature, phone_number, service_provided, source, url_hash, url, add_time, company_name) values('\
                     ...
                     + str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')) + "', '" +  data['company_name'] + "')"
