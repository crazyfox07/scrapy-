...
class BaiXing(object):
    ...
    def getCitys(self, url):
        ...
        try:
            ...
            citys = [(item.get_text(), urlparse.urljoin('http://', item.get('href'))) for li in soup.find_all('ul', class_ = 'wrapper') for item in li.find_all('a')]
    ...
    def spider(self, host, url):
        ...
        while(next_page_url != None):
            try:
                ...
                begin = time.time()
                r = self._s.get(url, proxies = proxies, headers = self._headers, timeout = 10)
